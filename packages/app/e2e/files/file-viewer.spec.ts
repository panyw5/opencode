...
test("markdown files open in preview mode by default", async ({ page, gotoSession }) => {
  await gotoSession()
  ...
  await expect
    .poll(
      async () => {
        const keys = await items.evaluateAll((nodes) => nodes.map((node) => node.getAttribute("data-key") ?? ""))
        index = keys.findIndex((key) => /README\.md$/i.test(key.replace(/^file:/, "")))
        return index >= 0
      },
      { timeout: 30_000 },
    )
  ...
  const viewer = page.locator('[data-component="file"][data-mode="markdown"]').first()
  await expect(viewer).toBeVisible()
  await expect(viewer.locator('[data-slot="file-markdown-preview"]')).toBeVisible()
  await expect(viewer.getByText("OpenCode Studio")).toBeVisible()
})
