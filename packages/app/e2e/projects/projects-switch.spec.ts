  ...
  try {
    await withProject(
      async ({ directory, gotoSession, trackSession }) => {
        ...
        await expect(currentButton).toHaveAttribute("data-session-current", "true")
        await expect(currentButton).toHaveAttribute("data-sidebar-expanded", "true")
        await expect(currentButton).toHaveAttribute("data-sidebar-floating", "true")
        await expect.poll(async () => (await currentButton.boundingBox())?.width ?? 0).toBeGreaterThan(45)
        await expect(otherButton).toBeVisible()
        await otherButton.click()

        await expect(page).toHaveURL(originalURL)
        await expect(otherButton).not.toHaveAttribute("aria-current", "true")
        await expect(otherButton).toHaveAttribute("data-session-current", "false")
        await expect(otherButton).toHaveAttribute("data-sidebar-expanded", "true")
        await expect(otherButton).toHaveAttribute("data-sidebar-floating", "true")
        await expect(currentButton).toHaveAttribute("data-session-current", "true")
        await expect(currentButton).toHaveAttribute("data-sidebar-expanded", "false")
        await expect(currentButton).toHaveAttribute("data-sidebar-floating", "false")
        await expect.poll(async () => (await currentButton.boundingBox())?.width ?? 0).toBeLessThanOrEqual(40)
        await expect.poll(async () => (await otherButton.boundingBox())?.width ?? 0).toBeGreaterThan(45)
        await expect(page.locator('[data-component="sidebar-panel"]').first()).toContainText(other)
        ...
        await expect(otherButton).toHaveAttribute("data-session-current", "true")
        await expect(otherButton).toHaveAttribute("data-sidebar-expanded", "true")
        await expect(otherButton).toHaveAttribute("data-sidebar-floating", "true")
  ...
  try {
    await withProject(
      async ({ directory }) => {
        ...
        await expect(currentButton).toHaveAttribute("data-sidebar-floating", "false")
        await expect.poll(async () => (await otherButton.boundingBox())?.width ?? 0).toBeGreaterThan(45)
        await expect.poll(async () => otherButton.evaluate((el) => getComputedStyle(el).boxShadow)).not.toBe("none")
      },
      { extra: [other] },
    )
  } finally {
