...
export async function loadRootSessions(input: RootLoadArgs) {
  const directory = input.directory.replace(/\\/g, "/")
  const timeoutMs = input.timeoutMs ?? DEFAULT_ROOT_SESSION_LOAD_TIMEOUT_MS
  let timer: ReturnType<typeof setTimeout> | undefined
  const timeout = new Promise<never>((_, reject) => {
    timer = setTimeout(() => reject(new RootSessionLoadTimeoutError(directory, timeoutMs)), timeoutMs)
  })
  ...
  try {
    const result = await Promise.race([input.list({ directory, roots: true }), timeout])
    return {
      data: result.data,
    } as const
  } finally {
