...
export const useSessionCommands = (actions: SessionCommandContext) => {
  ...
  const viewCommands = createMemo(() => [
    ...
    terminalCommand({
      ...
      disabled: platform.platform !== "desktop" || !platform.openPath || !server.isLocal(),
      onSelect: () => {
        const directory = sdk.directory
        if (!directory) return
        Promise.resolve(platform.openPath?.(directory, "Ghostty")).catch((err: unknown) => {
          showToast({
            variant: "error",
    ...
    terminalCommand({
      ...
      disabled: platform.platform !== "desktop" || !platform.openInEditor || !server.isLocal(),
      onSelect: () => {
        const directory = sdk.directory
        if (!directory) return
