...
const api: ElectronAPI = {
  ...
  renameLocalFile: (oldPath, newPath) => ipcRenderer.invoke("rename-local-file", oldPath, newPath),
  ...
  cliAgents: {
    ...
    getDshApiKey: (config) => ipcRenderer.invoke("cli-agents-dsh-api-key-get", config),
    listDshPlugins: (config, profile) => ipcRenderer.invoke("cli-agents-dsh-plugins-list", config, profile),
    setDshPluginEnabled: (config, input) => ipcRenderer.invoke("cli-agents-dsh-plugin-set-enabled", config, input),
  },
