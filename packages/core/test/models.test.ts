...
import { readFile, readdir, rm, writeFile, utimes, mkdir } from "fs/promises"
...
describe("ModelsDev Service", () => {
  ...
  it.live("refresh(true) fetches via HttpClient and updates the cache", () =>
    Effect.gen(function* () {
      ...
      expect(final.calls[0].userAgent).toContain(`/${Flag.OPENCODE_CLIENT}`)
    }),
  )
  ...
  it.live("refresh preserves a valid cache when the server returns malformed JSON", () =>
    Effect.gen(function* () {
      yield* writeCache(fixture)
      const original = yield* Effect.promise(() => readFile(cacheFile, "utf8"))
      ...
      expect(yield* Effect.promise(() => readFile(cacheFile, "utf8"))).toBe(original)
      const entries = yield* Effect.promise(() => readdir(Global.Path.cache))
      expect(entries.some((entry) => entry.startsWith("models.json.") && entry.endsWith(".tmp"))).toBe(false)
    }),
  )
})
