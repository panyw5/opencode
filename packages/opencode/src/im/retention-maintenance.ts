...
export async function start(policies: readonly RetentionPolicy[], cadence = Duration.days(1)): Promise<MaintenanceHandle | undefined> {
  if (!policies.length) return undefined
  const tick = () =>
    IMRetention.runtime.runPromise((service) => service.cleanup(policies))
  await tick()
    ...
    Effect.sleep(cadence).pipe(
      Effect.andThen(
        Effect.forever(
          ...
          Effect.catchCause((cause) => Effect.sync(() => log.error("IM retention maintenance tick failed", { cause }))),
