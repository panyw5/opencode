...
### OpenClaw、Hermes、GenericAgent 集成
...
- **背景 shell 和智能体**: 将 shell 命令和子智能体发送到后台背景运行，完成后自动注入到主智能体的对话流
- **异步停止**: 等待工具调用、思考或者回复完成后优雅停止
- **定时任务**: 支持定时任务功能，并将定时任务 API 暴露给智能体使用
- **项目任务** 创建项目跨会话任务、会话级 Todo 层级系统，API 暴露给智能体使用
- **Codex、claude、grok、dsh 外部参谋**: 接入 Codex 等作为外部智能体，可在会话中通过主智能体调用作为外部参谋
- **数学模式**: 内置了数学智能体框架 [Danus](https://github.com/frenzymath/Danus)，高效完成复杂、长程的数学任务
...
- **项目任务与会话级 Todo 面板**: 管理挂载项目级任务，查看会话级 Todo
...
- **添加临时附件**: 对话中快速添加临时附件并保存到临时文件夹
- **阅读模式**: 一键进入专注阅读模式，排除干扰


#### 配置面板
- 提供商、智能体、技能、命令、插件、MCP 一站式配置
- IM 通道、外部智能体配置
